...
CryptoCore.prototype.Reload = function (ifs) {
	let byteshex = bip39.mnemonicToEntropy(ifs.config.get(['vault', 'cryptobox_seed']));
	this.kp = load_keypair_from_seed_hex(byteshex);
};

CryptoCore.prototype.vault = function (d) {
	try {
		return encrypt(this.kp.publicKey, JSON.stringify(d));
	} catch (err) {
		return Promise.reject({message: `Failed to vault: ${err.message}`});
	}
};
CryptoCore.prototype.unvault = function (vaultedString) {
	try {
		return JSON.parse(decrypt(this.kp.publicKey, this.kp.privateKey, vaultedString));
	} catch (err) {
		return Promise.reject({message: `Failed to unvault: ${err.message}`});
	}
};
...
// passwordHash (password, oldhash)
// checkPasswordHash same as above but returns true/false?
// $pbkdf2-sha256$6400$0ZrzXitFSGltTQnBWOsdAw$Y11AchqV4b0sUisdZd0Xr97KWoymNE0LNNrnEgY4H9M
// $algo$rounds$salt$hash
function passwordHash (password, oldhash) {
	let rounds;
	let salt;
	if (oldhash) {
		const d = oldhash.split('$');
		salt = Buffer.from(d[3], 'base64');
		rounds = parseInt(d[2]);
	} else {
		salt = crypto.randomBytes(18);
		rounds = 6000;
	}

	const hash = crypto.pbkdf2Sync(Buffer.from(password, 'utf-8'), salt, rounds, 32, 'sha256');
	return `$pbkdf2-sha256$${rounds}$${salt.toString('base64')}$${hash.toString('base64')}`;
}

CryptoCore.load_keypair_from_seed_hex = load_keypair_from_seed_hex;
CryptoCore.encrypt = encrypt;
CryptoCore.decrypt = decrypt;
CryptoCore.passwordHash = passwordHash;
